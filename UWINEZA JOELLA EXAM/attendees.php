...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <label for="atid">AttendeesID:</label>
        ...
        <input type="text" id="occ" name="occ" required><br><br>

        <label for="rdt">RegistrationDate:</label>
        <input type="date" id="rdt" name="rdt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO attendees(AttendeesID,id, Occupation, RegistrationDate ) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $atid, $id, $occ, $rdt );
     ...
     include('database_connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $atid = $row['AttendeesID']; // Fetch the AttendeesID
                echo "<tr>
                    <td>" . $row['AttendeesID'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['Occupation'] . "</td>
                    <td>" . $row['RegistrationDate'] . "</td>
                    <td><a style='padding:4px' href='delete_attendees.php?AttendeesID=$atid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_attendees.php?AttendeesID=$atid'>Update</a></td> 
                </tr>";
            }
        } else {
