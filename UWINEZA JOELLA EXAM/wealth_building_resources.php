...
<head>
  ...
  <title>Our wealth_building_resources</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
   ...
   <h1><u> wealth_building_resources Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="rid">ResourceID:</label>
        <input type="number" id="rid" name="rid"><br><br>

        <label for="ttl">Title:</label>
        <input type="text" txt="ttl" name="ttl"><br><br>

        <label for="dpt">Description:</label>
        <input type="text" id="dpt" name="dpt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO wealth_building_resources(ResourceID,Title,Description,type) VALUES (?,?,?,?)");
    $stmt->bind_param("ssss",$rid,$ttl,$dpt,$typ );
...
$sql = "SELECT * FROM wealth_building_resources";
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of wealth_building_resources</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of wealth_building-resources</h2></center>
    <table border="5">
        <tr>
            <th>ResourceID</th>
            <th>Title</th>
       ...
       include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all wealth_building-resources
        $sql = "SELECT * FROM wealth_building_resources";
        $result = $connection->query($sql);

        // Check if there are any wealth_building_resources
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $rid = $row['ResourceID']; // Fetch the ResourceID
                echo "<tr>
                    <td>" . $row['ResourceID'] . "</td>
                    <td>" . $row['Title'] . "</td>
                    <td>" . $row['Description'] . "</td>
                    <td><a style='padding:4px' href='delete_wealth_building_resources.php?ResourceID=$rid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_wealth_building_resources.php?ResourceID=$rid'>Update</a></td> 
                </tr>";
            }
        } else {
