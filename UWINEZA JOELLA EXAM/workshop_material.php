...
<head>
  ...
  <title>Our workshop_material</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
    ...
    <h1><u> Workshop_material Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="mid">MaterialID:</label>
        ...
        <input type="text" id="mty" name="mty" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO workshop_material(MaterialID,WorkshopID,MaterialType) VALUES (?,?,?)");
    $stmt->bind_param("iis",$mid ,$wid,$mty );
...
$sql = "SELECT * FROM workshop_material";
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of workshop_material</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of workshop_material</h2></center>
    <table border="5">
        <tr>
            <th>MaterialID</th>
            <th>WorkshopID</th>
      ...
      include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all workshop_material
        $sql = "SELECT * FROM workshop_material";
        $result = $connection->query($sql);

        // Check if there are any workshop_material
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $mid = $row['MaterialID']; // Fetch the MaterialID
                echo "<tr>
                    <td>" . $row['MaterialID'] . "</td>
                    <td>" . $row['WorkshopID'] . "</td>
                    <td>" . $row['MaterialType'] . "</td>
                    <td><a style='padding:4px' href='delete_workshop_material.php?MaterialID=$mid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_workshop_material.php?MaterialID=$mid'>Update</a></td> 
                </tr>";
            }
        } else {
